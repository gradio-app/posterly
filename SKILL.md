...
```
   HTML (with @page { size: W H })
     ...
     │
     ▼  data-measure-role tags identify columns/footer-strip
     │
...
Use `templates/landscape_4col_neutral.html`, edit its `:root` design tokens for
the paper/reproduction, and fill TODO placeholders from the logged evidence.
...
| Body cols | 4 | Four balanced columns. |
...
- **Layout:** use the four-column landscape layout. Balance 3–5 substantive
  cards per column; do not select or create another layout.
...
1. `cp templates/landscape_4col_neutral.html <work-dir>/poster.html`.
...
- **`spread < 5 px`** across the last-card-bottoms of all four columns. Aim `< 3 px`.
...
- **`canvas-fill ∈ [95 %, 101 %]`** (coarse early diagnostic) — `[data-measure-role="poster"]` width/height ratio against the print viewport. Fires before the position check when the ratio is FAR off, with a more diagnostic error message that points at the common `@media print { :root { --u: 1mm } }` omission (renders at ~42 %) or hardcoded `width > @page` (renders at >100 %). For borderline 95–99 % cases, position-align is the truth. Tune via `--min-canvas-fill` / `--max-canvas-fill`. **Safe-area design** belongs as internal padding on a full-bleed `.poster` so the fixed canvas remains aligned to the page.
...
- gap > 50 everywhere: grow a card with substance (per Gate C / *Fill means substance*) or remove an optional block — don't leave the whitespace.
...
Checks: page count == 1, dimensions match the 60×36in canvas, file size ≤ limit. `--from-html <path>` reads `@page { size: … }` from the HTML so they can't drift apart.
...
All scripts read the 60×36in `@page` declaration from the input HTML.
...
13 rules: colors only via `var(--…)` from the `:root` token block (no stray hex), no inline `style=`, no gradients, font-family against a whitelist, font-size only from the `--fs-*` scale, bounded token count, the `data-*` / inline-SVG contracts, and (rule 13) every `block--modifier` variant class used in the markup must have a matching CSS rule — a dropped rule leaves the class inert and the layout silently wrong (for example, a `keybox--4` with no matching component rule can orphan a tile into an empty second row). Pure static analysis plus a small Playwright render gate for computed-style rules, so it's cheap — run it right after the Step 3 scaffold and on every layout change.
...
- **Prefer the named knobs.** Every fix inside the loop should be one of the 7 operations catalogued in `templates/COMPONENTS.md` (edit a `:root` token; swap/add/remove a catalogued component; rebalance paper-sourced content; edit a component's token-only CSS; toggle a predefined variant; fix an asset). These are *named, reusable* knobs, not one-off hacks.
...
- **Round caps are a guide, not a wall.** Default: ≤3 issues per round, and after ~3 rounds without reaching your visual bar, stop patching and escalate (revisit content distribution or ask for a human call) rather than endless cosmetic micro-tuning. Adjust the caps deliberately — you're in the loop.
...
Use only `templates/landscape_4col_neutral.html` (60×36in, four columns). It is tokenized and passes `style_check` as shipped.
