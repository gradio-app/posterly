...
**Lone wrapped fragment — display text that breaks to a one-word last line.** The orphan logic extends from stat numbers to *any short display text that wraps* — a title, `.section-title`, banner headline, or one-line takeaway whose break strands a single short token alone on the final line. The worst case is when that lone token is a **word carrying a leading footnote/superscript marker** (a `*`- or `†`-prefixed term): stranded alone and left-aligned, the marker+word reads as a stray symbol or a typo, not a word. `polish` does **not** see this — it inspects stat/num elements for a *trailing* glyph, not the wrap geometry of a heading — so guard it by hand: (1) **`text-wrap: balance`** on the heading evens the lines so no single word is stranded (it's the default on centered display text — keep it; don't pair `balance` with `text-align: left`, per the wrap rules above); (2) **`&nbsp;` glue** so the marker and enough of its phrase ride together onto the wrapped line (bind it to the word before it, `…&nbsp;*term`, so the marker can't open a line alone); (3) **reword** so the break falls at a natural phrase boundary. Never ship a one-word last line, and never a lone `*` / `†` / `‡` fragment.
